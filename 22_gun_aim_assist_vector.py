...
# พื้น uncertainty ของจุด intercept จาก 'sensor noise × latency' (deg)
# เดิม 0.05 = ค่า sim-quality (detection เป๊ะ) → บนโดรนจริง uncert ประเมินต่ำเกิน → 'มั่นใจ' ผิด
# ที่ระยะไกล → ยิงแล้วพลาด. จึงตั้ง 0.6 เพื่อให้ gate ยอมยิงเฉพาะตอน hit_r ใหญ่พอ (= ระยะใกล้)
#
# ⚠️ ค่านี้สร้าง 'กำแพงระยะ': floor เป็นค่าคงที่ แต่ hit_r = atan(0.35/R) เล็กลงตามระยะ
#    → เลยระยะหนึ่ง uncert > hit_r × K เสมอ = ยิงไม่ได้เลย ต่อให้ track เนี๊ยบแค่ไหน
#    ระยะไกลสุดที่ยิงได้ = 0.35 / tan(floor / LOCK_FIRE_CONFIDENCE_K)
#      floor 0.60 -> 23 m   |  0.42 -> 33 m  |  0.30 -> 47 m  |  0.20 -> 70 m
#    ระบบ print ค่านี้ตอน startup แล้ว (ดู 'fire gate: max range') — จะได้ไม่มีกำแพงล่องหนอีก
#
# ทำไม 0.6 -> 0.42: ค่า 0.6 ถูก 'จูน' ตอนที่ estimate_distance_m ยังอ่านระยะสั้นกว่าจริง ~30%
# (บั๊ก FOV 60° ทั้งที่จริง 44°) → hit_r ถูกพองไป 1.43x → floor ที่ fit ได้จึงพองตามไปด้วย
# พอแก้ FOV ให้ระยะถูก กำแพงเลยหดจาก ~33m เหลือ 23m = 'ไล่ตามแต่ไม่ยิง' โดรนบินหนี
# หาร 1.43 กลับ → ได้พฤติกรรมยิง/ไม่ยิงที่ 'ระยะจริง' เท่าเดิม บนฟิสิกส์ที่ถูกต้องแล้ว
#
# ค่านี้ยัง 'เดา' อยู่ ไม่เคยวัด — 0.6° = 52px ของ bbox jitter ซึ่งสูงกว่าที่ระบบสมมติเอง
# (LOCK_MEAS_SIGMA_PX = 5px) ถึง 10 เท่า. กด W (wizard) ขั้น noise เพื่อ 'วัดจริง'
# ถ้า jitter จริงต่ำ ลด floor ได้อีก → ยิงได้ไกลขึ้นมาก
LOCK_FIRE_NOISE_FLOOR_DEG = 0.42
...
# วัด sensor noise จริงตอน LOCK (แทนที่จะเดา LOCK_FIRE_NOISE_FLOOR_DEG / LOCK_MEAS_SIGMA_PX)
_lock_meas_hist = deque(maxlen=3)      # (b_pan, b_tilt, t) 3 ตัวล่าสุด → second difference
_lock_meas_resid = deque(maxlen=900)   # σ ต่อเฟรม ย้อนหลัง ~60 วิ ที่ 15fps
_lock_range_hist = deque(maxlen=120)   # ระยะที่ประเมินได้ — ดูว่ามันนิ่งหรือกระโดด (bbox ไม่เสถียร)
...
def fire_gate_max_range_m() -> float:
    """ระยะไกลสุดที่ fire gate 'ยอมให้ยิงได้' ด้วยค่าปัจจุบัน (ต่อให้ track เนี๊ยบ resid=0)

    gate เปิดเมื่อ uncert ≤ hit_r × K  โดย uncert ≥ NOISE_FLOOR เสมอ และ
    hit_r = max(MIN_RADIUS, atan(HIT_RADIUS_M / R)) ซึ่งเล็กลงตามระยะ
    → มีระยะหนึ่งที่ hit_r × K < NOISE_FLOOR เสมอ = ยิงไม่ได้เลย
    ฟังก์ชันนี้ทำให้ 'กำแพง' นั้นมองเห็นได้ แทนที่จะซ่อนอยู่จนงงว่าทำไมไม่ยิง
    """
    need_deg = LOCK_FIRE_NOISE_FLOOR_DEG / max(1e-6, LOCK_FIRE_CONFIDENCE_K)
    if need_deg <= LOCK_FIRE_READY_MIN_RADIUS_DEG:
        return float("inf")          # floor ต่ำมาก → ไม่มีกำแพง
    t = math.tan(math.radians(need_deg))
...
class ShooterConfig:
    ...
    def __init__(self, camera_name: str = "cam4"):
        ...
        # ระยะที่ 'zero' crosshair ไว้ (เมตร) — 0 = boresight เป็นมุมกลไกล้วน ไม่มี drop อบอยู่
        # ถ้า zero โดยยิงเป้าจริงที่ระยะ D → drop ที่ระยะ D ถูกอบเข้าไปใน boresight แล้ว
        # → ตอนยิงระยะ R ต้องชดเชยแค่ 'ส่วนต่าง' drop(R) − drop(D) ไม่ใช่ drop(R) เต็ม ๆ
        # (ไม่งั้นนับซ้ำ: ที่ระยะ zero เองก็ยังเงยเกินไป drop(D))
        self.boresight_zero_range_m = 0.0
    ...
    def load(self):
        ...
        if os.path.isfile(self._boresight_path):
            try:
                ...
                self.boresight_zero_range_m = float(b.get("zero_range_m", 0.0) or 0.0)
                return
            except Exception:
                pass
        ...
        try:
            ...
            with open(self._boresight_path, "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "zero_range_m": self.boresight_zero_range_m,
                }, f, indent=2)
        except Exception:
            pass
...
def _lock_feed_bearing_measurement(
    ...
    _post_p, _post_t = lock_kalman.update(b_pan, b_tilt, float(conf))

    # --- วัด 'sensor noise จริง' แบบ passive ระหว่างใช้งาน (second difference) ---
    # ห้ามใช้ (measurement − Kalman posterior): posterior 'ตามไม่ทัน' เป้าที่กำลังเคลื่อน
    # (Q ตั้งไว้ต่ำ → filter แข็ง) → residual จะโตตามความเร็วเป้า = วัด lag ไม่ใช่วัด noise
    # (ลองแล้วได้ σ 2.1° บนเป้าที่วิ่ง 8°/s ซึ่งมั่ว)
    #
    # ใช้ second difference แทน: ถ้า b(t) = p + v·t + n(t)  (เคลื่อนที่เชิงเส้น + noise)
    #   b[k] − 2·b[k−1] + b[k−2] = n[k] − 2·n[k−1] + n[k−2]      ← พจน์ p และ v หายหมด
    #   var ของมัน = 6σ²  →  σ = rms(d²) / √6
    # เหลือแต่ noise จริง + ความเร่ง (ซึ่งเล็กกว่ามากในกรอบเวลา 1-2 เฟรม)
    _h = _lock_meas_hist
    _h.append((b_pan, b_tilt, t_capture))
    if len(_h) >= 3:
        (p0, t0, _), (p1, t1, _), (p2, t2, _) = _h[-3], _h[-2], _h[-1]
        d2_pan = p2 - 2.0 * p1 + p0
        d2_tilt = t2 - 2.0 * t1 + t0
        _lock_meas_resid.append(math.hypot(d2_pan, d2_tilt) / math.sqrt(6.0))
    return True


def lock_range_jitter() -> Optional[Tuple[float, float]]:
    """(median ระยะ, ความกระเพื่อม %) — bbox ไม่เสถียร → ระยะกระโดด → hit_r กระโดด → gate กะพริบ
    ถ้าค่านี้สูง (>30%) ปัญหาอยู่ที่ 'detection ไม่นิ่ง' ไม่ใช่ที่ fire gate"""
    if len(_lock_range_hist) < 20:
        return None
    a = np.asarray(_lock_range_hist, dtype=np.float64)
    med = float(np.median(a))
    if med <= 1e-6:
        return None
    # ใช้ MAD (median absolute deviation) — ทนต่อ outlier กว่า std
    mad = float(np.median(np.abs(a - med)))
    return med, 100.0 * mad / med


def lock_noise_sigma_deg() -> Optional[float]:
    """σ ของ sensor noise ที่ 'วัดได้จริง' ตอน LOCK (องศา) — None ถ้าตัวอย่างยังน้อย

    ใช้ median ไม่ใช่ mean: bbox กระโดด/สลับเป้าเป็นครั้งคราวจะดัน mean ขึ้นมหาศาล
    median ทนต่อ outlier พวกนั้น → ได้ noise 'ปกติ' ที่เป็นตัวแทนจริง
    """
    if len(_lock_meas_resid) < 30:
        return None
    return float(np.median(np.asarray(_lock_meas_resid, dtype=np.float64)))
...
def compute_firing_lead(vpan_deg_s, vtilt_deg_s, range_m, muzzle_ms,
    ...
    if drop_comp:
        ...
        # หัก drop ที่ 'ระยะ zero' ออก — ค่านั้นถูกอบอยู่ใน boresight แล้วตอน zero crosshair
        # ด้วยการยิงเป้าจริง ถ้าไม่หัก = นับซ้ำ (ที่ระยะ zero เองก็จะยังเงยเกินไป drop(zero))
        # zero_range_m = 0 → boresight เป็นมุมกลไกล้วน → ชดเชย drop เต็มตามปกติ
        if zero_range_m and zero_range_m > 0:
            t0 = zero_range_m / muzzle_ms
            drop0_m = 0.5 * GRAVITY_MS2 * t0 * t0
            drop_deg -= math.degrees(math.atan2(drop0_m, zero_range_m))
...
def _tick_lock(ctx):
    ...
    if fire_range_m:
        _lock_range_hist.append(float(fire_range_m))
    ...
    if LOCK_FIRE_SOLUTION_ENABLED and _lead_active:
        ...
        _flead_p, _flead_t, fire_t_flight, _drop = compute_firing_lead(
            ...
            zero_range_m=float(ctx.get("boresight_zero_range_m", 0.0) or 0.0),
        )
    ...
    if LOCK_TRACK_DEBUG and (now - _lock_dbg_last_t[0]) > 0.3:
        ...
        # σ ที่ 'วัดได้จริง' — เทียบกับ noise floor ที่ 'เดา' ไว้ ให้เห็นว่าห่างกันแค่ไหน
        _sig = lock_noise_sigma_deg()
        _sig_s = f" sig={_sig:.3f}°/floor={LOCK_FIRE_NOISE_FLOOR_DEG}" if _sig is not None else ""
        _rj = lock_range_jitter()
        if _rj is not None:
            _sig_s += f" rng_jit={_rj[1]:.0f}%" + ("⚠bbox-ไม่นิ่ง" if _rj[1] > 30 else "")
        print(f"[LOCK]{_sig_s} aim=({aim_pan:+6.2f},{aim_tilt:+6.2f}) cur=({pan_cur:+6.1f},{tilt_cur:+6.1f}) "
              ...
              # บอกตรง ๆ ว่า gate ปิดเพราะ 'เป้าไกลเกินกำแพง' ไม่ใช่เพราะ track ไม่ดี
              # (เป็นคนละเรื่องกันโดยสิ้นเชิง แต่บนหน้าจอดูเหมือนกันหมด = 'ไล่ตามแต่ไม่ยิง')
              + (f" ⛔OUT-OF-ENVELOPE(>{fire_gate_max_range_m():.0f}m: ลด noise floor)"
                 if (not confident_ready and hit_radius_deg is not None
                     and LOCK_FIRE_NOISE_FLOOR_DEG > hit_radius_deg * LOCK_FIRE_CONFIDENCE_K)
                 else "")
...
def main():
    ...
    _fg_max = fire_gate_max_range_m()
    print(
        f"Gun Aim Assist: fire gate — noise_floor={LOCK_FIRE_NOISE_FLOOR_DEG}° K={LOCK_FIRE_CONFIDENCE_K} "
        f"→ ยิงได้ไกลสุด {'ไม่จำกัด' if _fg_max == float('inf') else f'{_fg_max:.0f} m'} "
        f"(ไกลกว่านี้ gate ไม่มีทางเปิด — ลด noise floor ในหน้า S หรือกด W วัดจริง)",
        flush=True,
    )
