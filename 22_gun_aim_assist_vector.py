...
LOCK_KALMAN_COAST_MAX_SEC = 0.5   # coast ต่อได้นานสูงสุดตอน YOLO หลุด (ไม่ต้อง re-lock)
...
LOCK_LEAD_COAST_SWITCH_SEC = 0.2  # age เกินค่านี้ = ถือว่าเป้าหลุด → ใช้ bounded coast แทน linear lead
...
class _SimpleIoUTracker:
    ...
    def update(
        ...
        candidates.sort(key=lambda x: (x[1], -x[2]))  # dist น้อยก่อน, iou มากเป็น tiebreak
        best_det = candidates[0][0] if candidates else None
...
def _lock_feed_bearing_measurement(
    ...
    if (
        ...
        return False
    pose_cap = pose_hist.pose_at(t_capture) if pose_hist is not None else None
    if pose_cap is None:
        pose_cap = (
...
def _tick_lock(ctx):
    ...
    if _lead_active:
        ...
        if ld_mag > LOCK_LEAD_MAX_DEG and ld_mag > 1e-6:
            ...
            aim_pan = base_pan + ld_p * _s
            aim_tilt = base_tilt + ld_t * _s
    else:
...
def main():
    ...
    if os.environ.get("LOCK_LOG") == "1":
        try:
            ...
            _lock_log.write("# t_iso,yolo_fps,det_rate,match_rate,motion_hz,lost_evt,best_conf,imgsz,arm_pan,arm_tilt,lat_ms\n")
            _lock_log.flush()
            print("[LOCKLOG] เปิด — บันทึกลง lock_diag.log (เฉพาะตอนอยู่โหมด LOCK)")
        except Exception as _e:
            print(f"[LOCKLOG] open failed: {_e}")
    _lld = {"yolo": 0, "real": 0, "match": 0, "motion": 0, "lost": 0, "bestconf": 0.0, "t": time.time()}
            ...
            if network_loss_mode:
                ...
                if (LOCK_MOTION_ASSIST_ENABLED and arm_mode_manager is not None
                    ...
                    if (not _yolo_matched_frame and _lock_prev_gray is not None
                        ...
                        if _mres is not None:
                            ...
                            _lock_feed_bearing_measurement(
                                lock_iou_tracker, lock_kalman, lock_arm_pose_hist, arm_controller,
                                time.time(), cx_frame, cy_frame, px_per_deg_x, px_per_deg_y,
                            )
                            if _lock_log is not None:
                                _lld["motion"] += 1
                ...
                if _lock_log is not None and arm_mode_manager is not None and arm_mode_manager.mode == MODE_LOCK:
                    ...
                    if _now_ll - _lld["t"] >= 1.0:
                        ...
                        _lock_log.flush()
                        _lld = {"yolo": 0, "real": 0, "match": 0, "motion": 0, "lost": 0, "bestconf": 0.0, "t": _now_ll}
