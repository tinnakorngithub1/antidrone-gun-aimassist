...
# Q/R scale จากค่า pixel-space เดิมด้วย (1/ppd)² (ppd≈87 px/deg) เพื่อคง bandwidth ของ filter:
LOCK_BEARING_KALMAN_Q = KALMAN_PROCESS_NOISE_Q / (87.0 ** 2)      # ≈1.3e-5 deg²
LOCK_BEARING_KALMAN_R = KALMAN_MEASUREMENT_NOISE_R / (87.0 ** 2)  # ≈3.3e-3 deg² (σ≈5px≈0.06°)
...
# Slew-aware: ตอนแขนหมุน (กล้องเลื่อน > IOT_SLEW_BLIND_PX/เฟรม) YOLO บอดเพราะภาพเบลอ = เรื่องปกติ
# → นับ miss แค่ IOT_SLEW_MISS_WEIGHT (ไม่ประกาศ lost ระหว่าง slew, coast เล็งตาม bearing ต่อ)
IOT_SLEW_BLIND_PX = 22          # กล้องเลื่อนเกินนี้/เฟรม = แขนกำลัง slew (≈0.25°) → ถือว่า YOLO บอดคาดเดาได้
...
LOCK_MOTION_SEARCH_R = 110      # รัศมี search window (px, full-res) รอบตำแหน่งที่ทำนาย
...
def _lock_motion_track(gray_cur, gray_prev, pred_cx, pred_cy, sx, sy,
                       search_r=LOCK_MOTION_SEARCH_R, diff_thresh=LOCK_MOTION_DIFF_THRESH,
...
IOT_ASSOC_MIN_PX = 40           # รัศมีขั้นต่ำ (px) กันเป้าเล็กจน gate แคบเกิน
...
class _SimpleIoUTracker:
    ...
    def __init__(self) -> None:
        ...
        self._arm_moved_px: float = 0.0          # กล้องเลื่อนกี่ px เฟรมล่าสุด (จาก ego-comp)
    ...
    def _register_miss(self) -> None:
        ...
        _slewing = self._arm_moved_px > IOT_SLEW_BLIND_PX
    ...
    def update(
        ...
        if (cur_pan is not None and cur_tilt is not None and ppd_x and ppd_y
            ...
            shift_x = -(float(cur_pan) - self._last_pan) * float(ppd_x)
            shift_y = -(float(cur_tilt) - self._last_tilt) * float(ppd_y)
            self._arm_moved_px = math.hypot(shift_x, shift_y)   # ใช้ตัดสินว่าแขนกำลัง slew (miss ถ่วงน้ำหนัก)
        ...
        assoc_gate = max(IOT_ASSOC_MIN_PX,
...
LOCK_ROI_MISS_GROW_PX = 130   # ขยาย ROI (px) ต่อ 1 เฟรมที่ YOLO ไม่ match
LOCK_ROI_MAX_PX = 1920        # cap ขนาด ROI — ที่ max ยัง 'สี่เหลี่ยมจัตุรัส native' (aspect ถูก) ต่างจาก
                              # center crop เดิมที่บิด 3× ; = พื้นที่ค้นกว้างสุดโดยไม่เสีย aspect
...
def crop_roi_native(frame, cx, cy, target_size, bbox_diag=None, miss_run=0):
    """LOCK ROI crop: หน้าต่างสี่เหลี่ยมจัตุรัสรอบตำแหน่ง track (cx,cy) สำหรับส่งเข้า YOLO.
    - ROI = target_size พิกเซล 'จริง' (native) → ไม่ resize ไม่บิดสัดส่วน โดรนเล็กคงคมชัดเต็ม
    - เป้าใหญ่ (bbox_diag × LOCK_ROI_BBOX_MULT > target_size) → ขยาย ROI ให้ครอบเป้าแล้ว resize ลง
    ...
    if miss_run > 0:
        roi = int(roi + miss_run * LOCK_ROI_MISS_GROW_PX)
    roi = min(roi, LOCK_ROI_MAX_PX, w, h)  # cap + กันเฟรมเล็กกว่า ROI
...
def _cam4_crosshair_trim_deg(ctx: Dict[str, Any]) -> Tuple[float, float]:
    ...
    if w is None or h is None or cx_frame is None or cy_frame is None:
        return (0.0, 0.0)
    if ppx is None or ppy is None or float(ppx) <= 0.0 or float(ppy) <= 0.0:
        return (0.0, 0.0)
    geom_cx = float(w) * 0.5
    geom_cy = float(h) * 0.5
...
def _lock_feed_bearing_measurement(
    ...
    # → กัน arm-motion leak เข้า bearing ตอน slew (แขนไล่เลยเป้า). LOCK_EGO_COMP_LATENCY_SEC=0 = ปิด
    pose_cap = pose_hist.pose_at(t_capture - LOCK_EGO_COMP_LATENCY_SEC) if pose_hist is not None else None
    if pose_cap is None:
        pose_cap = (
...
def _tick_lock(ctx):
    ...
    # --- per-frame diag (LOCK_FRAME_LOG=1): แยกแยะว่า pan/tilt jump มาจากอะไร ---
    # arm(แขนจริง) vs base(ตำแหน่งเป้าปัจจุบันจาก KF) vs aim(คำสั่ง) vs meas(measurement+action)
    # ถ้า aim กระโดดแต่ base ไม่ = lead/firing_lead; ถ้า base กระโดด = KF/measurement; ถ้า meas=reseed = re-lock snap
...
def main():
    ...
    if os.environ.get("LOCK_FRAME_LOG") == "1":
        try:
            ...
            _lock_frame_log[0].write("# t,coast_ms,arm_pan,arm_tilt,base_pan,base_tilt,aim_pan,aim_tilt,vpan,vtilt,speed,lead_gain,fire_lead,meas_pan,meas_tilt,action\n")
            _lock_frame_log[0].flush()
    ...
    # LOCK mode: Kalman ใน bearing space (หน่วยองศา — Q/R scale จาก pixel ด้วย 1/ppd²)
    lock_kalman = _TargetKalman(q=LOCK_BEARING_KALMAN_Q, r=LOCK_BEARING_KALMAN_R)
    ...
    def _exit_pxdeg_calib(reload_calib=True):
        nonlocal app_mode, arm_calibration_ready, px_per_deg_x, px_per_deg_y
            ...
            if network_loss_mode:
                ...
                if arm_controller is not None and arm_mode_manager is not None and px_per_deg_x is not None and px_per_deg_y is not None and x_lo is not None:
                    _ctx = {
                        ...
                        "lock_range_m": (
                            estimate_distance_m(
                                lock_csrt_bbox[2], lock_csrt_bbox[3], w, h,
                                fov_h, fov_v, config.target_size_m
