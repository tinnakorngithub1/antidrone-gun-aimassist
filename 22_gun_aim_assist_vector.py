...
LOCK_FIRE_SAFE_ENABLED = True
LOCK_FIRE_SAFE_PAN_DEG = 45.0      # |target_pan| ต้อง ≤ ค่านี้ถึงจะยิงได้
LOCK_FIRE_SAFE_TILT_DEG = 25.0     # |target_tilt| ต้อง ≤ ค่านี้
...
IOT_MISS_FRAMES_TO_LOST = 3     # hysteresis: พลาดกี่ครั้งติดกัน (ผล YOLO ที่ไม่ match) ถึงประกาศ lost
...
class _SimpleIoUTracker:
    ...
    def _register_miss(self) -> None:
        """ไม่ match ในผล YOLO รอบนี้ — ประกาศ lost เมื่อพลาดติดกันครบ IOT_MISS_FRAMES_TO_LOST
        (กันเป้าวูบเฟรมเดียวแล้ว lost ทันที; ระหว่างรอ smooth_cx คงค่าล่าสุดไว้ให้ HUD)."""
        self.miss_count += 1
        if self.miss_count >= IOT_MISS_FRAMES_TO_LOST:
            self.lost = True
            ...
            if network_loss_mode:
                ...
                if _picked is not None:
                    ...
                    if arm_mode_manager is not None and arm_mode_manager.mode == MODE_LOCK and lock_iou_tracker.initialized and last_detections is not None:
                        _lock_matched, _ = lock_iou_tracker.update(last_detections, w, h)
                        if _lock_matched:
