...
YOLO_CONF_DETECT = 0.01
YOLO_CONF_LOCK = 0.01
YOLO_CONF_MIN = 0.01   # ใช้กับ detection (backward compat)
...
YOLO_ENGINE_640_PATH = "yolo_11n_day_night_200_2_imgsz640.engine" #yolo26_60epoch.pt  last_imgsz640.engine
...
TARGET_MIN_BOX_PX = 15  # พิกเซล — ถ้า min(w,h) < ค่านี้ไม่เลือกเป็นเป้า
...
def main():
    ...
    display_fps_times = deque(maxlen=30)
    loop_fps_times = deque(maxlen=30)
    prev_display_frame_ts = None
    display_fps = 0.0
    loop_fps = 0.0
    display_lag_ms = 0.0
    frame_counter = 0
    ...
    try:
        while True:
            t0 = time.time()
            active, frame, frame_timestamp = cam.read()
            ...
            if frame_timestamp is not None:
                ...
                display_lag_ms = max(0.0, (now_display - frame_timestamp) * 1000.0)
                if prev_display_frame_ts is not None and frame_timestamp > prev_display_frame_ts:
                    display_fps_times.append(frame_timestamp - prev_display_frame_ts)
                    if display_fps_times:
                        display_fps = 1.0 / (sum(display_fps_times) / len(display_fps_times))
                prev_display_frame_ts = frame_timestamp
            ...
            loop_fps_times.append(elapsed)
            if loop_fps_times:
                loop_fps = 1.0 / (sum(loop_fps_times) / len(loop_fps_times))
            ...
            disp_text = f"DISP:{display_fps:.1f}"
            (disp_tw, _), _ = cv2.getTextSize(disp_text, cv2.FONT_HERSHEY_SIMPLEX, 1.2, 2)
            cv2.putText(frame, disp_text, (w - disp_tw - 28, 42), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 0), 2)
            loop_text = f"LOOP:{loop_fps:.1f}"
            (loop_tw, _), _ = cv2.getTextSize(loop_text, cv2.FONT_HERSHEY_SIMPLEX, 0.75, 2)
            cv2.putText(frame, loop_text, (w - loop_tw - 28, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
            lag_text = f"LAG:{display_lag_ms:.0f}ms"
            (lag_tw, _), _ = cv2.getTextSize(lag_text, cv2.FONT_HERSHEY_SIMPLEX, 0.75, 2)
            cv2.putText(frame, lag_text, (w - lag_tw - 28, 98), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
            ...
            if pipeline_latency_history:
                ...
                cv2.putText(frame, lat_text, (w - lt_tw - 28, 126), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
            ...
            cv2.putText(frame, conf_text, (w - conf_tw - 28, 154), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
