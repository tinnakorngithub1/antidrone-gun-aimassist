...
# สลับส่ง UDP cue ระหว่างรัน (ไม่ชนกับคีย์อื่นในลูปหลัก: q y t u o p l i b g v h f c Enter …)
ARM_CUE_SEND_TOGGLE_KEY = "A"
...
try:
    ...
    def get_runtime_cleanup():
        class _NoopCleanup:
            def install(self): pass
            def set_resources(self, **_kwargs): pass
        return _NoopCleanup()

    def wait_for_camera_first_frame(cam, camera_name=None):
        for _ in range(90):
    ...
    def open_camera_with_retries(build_camera_fn, camera_name=None):
        cam = build_camera_fn(camera_name)
        cam.start()
        frame = wait_for_camera_first_frame(cam, camera_name)
        return cam, frame

    def cam_hud_label_and_color(cam, active=True, frame=None):
        return ("CAM:?", (120, 120, 120))
    ...
    def network_loss_placeholder(width, height, camera_name, status_text):
        ...
        cv2.putText(frame, f"{camera_name} OFFLINE", (20, h // 2), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
        return frame
else:
    SHOW_PERSISTENCE_PATHS = False
...
def main(camera_override=None, active_camera_name=None):
    ...
    if camera_override is not None:
        ...
        cam, frame = open_camera_with_retries(build_camera, camera_name)
        if frame is None:
            print("❌ Camera/Video failed to open. Exiting.")
            if cam is not None and hasattr(cam, "release"):
                cam.release()
            return
    runtime_cleanup.set_resources(cam=cam)
    ...
    show_profile_hud = True  # F = toggle per-stage timing (motion / YOLO / track+graph / display)
    ...
    # TG breakdown (ms): tracker+snap | graph_manager | distance | handoff | label_trainer | yolo_conf
    prof_sub_trk_ms = prof_sub_gm_ms = prof_sub_dist_ms = 0.0
    prof_sub_hof_ms = prof_sub_lbl_ms = prof_sub_ycf_ms = 0.0
    PROF_AVG_FRAMES = 20
    ...
    prof_hist_sub_trk = deque(maxlen=PROF_AVG_FRAMES)
    prof_hist_sub_gm = deque(maxlen=PROF_AVG_FRAMES)
    prof_hist_sub_dist = deque(maxlen=PROF_AVG_FRAMES)
    prof_hist_sub_hof = deque(maxlen=PROF_AVG_FRAMES)
    prof_hist_sub_lbl = deque(maxlen=PROF_AVG_FRAMES)
    prof_hist_sub_ycf = deque(maxlen=PROF_AVG_FRAMES)
    ...
    try:
        while True:
            ...
            if not active or frame is None:
                ...
                if hasattr(cam, "get_stream_health"):
                    try:
                        ...
                        pass
                placeholder_w, placeholder_h = last_good_w, last_good_h
                if hasattr(cam, "get_last_frame_size"):
                    try:
                        size = cam.get_last_frame_size()
                        if size and len(size) == 2:
                            placeholder_w = int(size[0]) or placeholder_w
                            placeholder_h = int(size[1]) or placeholder_h
                    except Exception:
                        pass
                frame = network_loss_placeholder(
                    placeholder_w,
                    placeholder_h,
                    camera_name,
                    stream_health,
                )
                network_loss_mode = True
            ...
            if not network_loss_mode:
                last_good_w, last_good_h = w, h
            ...
            if network_loss_mode:
                ...
                _hud_bar_h = max(28, int(40 * ui_scale))
                _hud_font = cv2.FONT_HERSHEY_SIMPLEX
                _hud_fs = max(0.4, round(0.55 * ui_scale, 2))
                _hud_th = max(1, int(round(2 * _hud_fs)))
                cv2.rectangle(frame, (0, h - _hud_bar_h), (w, h), (0, 0, 0), -1)
                cv2.putText(
                    frame,
                    _cam_lbl,
                    (max(8, int(16 * ui_scale)), h - max(8, int(12 * ui_scale))),
                    _hud_font,
                    _hud_fs,
                    _cam_col,
                    _hud_th,
            ...
            try:
                ...
                _t_before_gm = time.perf_counter()
                prof_sub_trk_ms = (_t_before_gm - _t_prof_tr0) * 1000.0
                ...
                _t_after_gm = time.perf_counter()
                prof_sub_gm_ms = (_t_after_gm - _t_before_gm) * 1000.0
                ...
                prof_sub_dist_ms = (_t_after_dist - _t_after_gm) * 1000.0
                ...
                if sky_mask is not None and grid_filter is not None:
                    ...
                    handoff_state = {}
                _t_after_hof = time.perf_counter()
                prof_sub_hof_ms = (_t_after_hof - _t_after_dist) * 1000.0
                ...
                _t_after_lbl = time.perf_counter()
                prof_sub_lbl_ms = (
                    (_t_after_lbl - _t_after_hof) * 1000.0 if label_trainer is not None else 0.0
                )

                # Match YOLO detections กับ tracked objects และอัปเดต confidence (ใช้ yolo_dets_this_frame จากก่อน graph)
                _t_before_ycf = time.perf_counter()
                ...
                prof_sub_ycf_ms = (time.perf_counter() - _t_before_ycf) * 1000.0
                ...
                if show_heavy_ui:
                    ...
                    y_pos_prof2 = None
                    y_pos_arm = None
                    _prof_font_scale = _prof2_font_scale = 0.35
                    _prof_font_th = _prof2_font_th = 2
                    x_start = max(4, int(20 * ui_scale))
                    margin_right = max(20, int(40 * ui_scale))
                    # Reserve space for HUD(H): ON/OFF at bottom-right (ARM cue ย้ายไปแถวใต้ PROF)
                    hud_h_reserved_w = max(140, int(220 * ui_scale))
                    available_width = w - x_start - margin_right - hud_h_reserved_w
                    n_slots = 12
                    ...
                    while font_scale >= min_font_scale:
                        ...
                        (w_cam, _), _ = cv2.getTextSize("CAM:WAIT", font, font_scale, thickness)
                        ...
                        total_w = w_box + w_paths + w_guides + w_cam + w_fps + w_lat + w_yolo + w_mode + w_label + w_queue + w_acc + w_yolo_px + (n_slots - 1) * spacing
                    ...
                    # PROF + แถว ARM: baseline จาก getTextSize (ARM ใต้ PROF บนแถว BOXES)
                    _hud_bar_base = max(20, int(60 * ui_scale))
                    hud_bar_height = _hud_bar_base
                    _row_gap = max(12, int(14 * ui_scale))
                    (_, _th_m), _ = cv2.getTextSize("FPS: 999.9", font, font_scale, thickness)
                    _arm_scale = max(0.28, round(font_scale * 0.95, 2))
                    _arm_th = max(1, int(round(1.6 * _arm_scale)))
                    _arm_sample = (
                        f"ARM CUE ({ARM_CUE_SEND_TOGGLE_KEY}) ON  "
                        "192.168.144.66:9999ms"
                    )
                    (_, _th_arm), _bl_arm = cv2.getTextSize(
                        _arm_sample, font, _arm_scale, _arm_th
                    )

                    if show_profile_hud:
                        _prof_font_scale = max(0.32, round(font_scale * 1.18, 2))
                        _prof_font_th = max(2, int(round(2 * _prof_font_scale)))
                        _prof2_font_scale = max(0.30, round(font_scale * 1.10, 2))
                        _prof2_font_th = max(2, int(round(2 * _prof2_font_scale)))
                        _pf_sizing = (
                            f"PROF(F): avg{PROF_AVG_FRAMES}  "
                            "M:999.9 Y:999.9 TG:999.9 D:999.9ms"
                        )
                        _tg_sizing = (
                            "TG: TRK:999.9 GM:999.9 DS:999.9 "
                            "HO:999.9 LB:999.9 YC:999.9ms"
                        )
                        (_, _th_p1), _bl_p1 = cv2.getTextSize(
                            _pf_sizing, font, _prof_font_scale, _prof_font_th
                        )
                        (_, _th_p2), _bl_p2 = cv2.getTextSize(
                            _tg_sizing, font, _prof2_font_scale, _prof2_font_th
                        )
                        if cue_sender is not None:
                            y_pos_arm = int(y_pos - _th_m - _row_gap - _bl_arm)
                            y_pos_prof = int(y_pos_arm - _th_arm - _row_gap - _bl_p1)
                        else:
                            y_pos_arm = None
                            y_pos_prof = int(y_pos - _th_m - _row_gap - _bl_p1)
                        y_pos_prof2 = int(y_pos_prof - _th_p1 - _row_gap - _bl_p2)
                        _prof_stack_top = min(y_pos_prof2 - _th_p2, y_pos_prof - _th_p1) - max(
                            6, int(8 * ui_scale)
                        )
                        if cue_sender is not None:
                            _prof_stack_top = min(
                                _prof_stack_top,
                                y_pos_arm - _th_arm - max(4, int(6 * ui_scale)),
                            )
                        hud_bar_height = min(
                            h - 2,
                            max(_hud_bar_base, h - int(_prof_stack_top)),
                        )
                    elif cue_sender is not None:
                        y_pos_arm = int(y_pos - _th_m - _row_gap - _bl_arm)
                        _prof_stack_top = y_pos_arm - _th_arm - max(6, int(8 * ui_scale))
                        hud_bar_height = min(
                            h - 2,
                            max(_hud_bar_base, h - int(_prof_stack_top)),
                        )
                    ...
                    # แถวสอง–สาม: เวลาแยกช่วง + แตก TG (สลับด้วย F)
                    if show_profile_hud and y_pos_prof is not None:
                        _pf_scale = _prof_font_scale
                        _pf_th = _prof_font_th
                        _pm = (
                        ...
                        if y_pos_prof2 is not None:
                            _pf2_scale = _prof2_font_scale
                            _pf2_th = _prof2_font_th
                            _atr = (
                                sum(prof_hist_sub_trk) / len(prof_hist_sub_trk)
                                if prof_hist_sub_trk
                                else 0.0
                            )
                            _agm = (
                                sum(prof_hist_sub_gm) / len(prof_hist_sub_gm)
                                if prof_hist_sub_gm
                                else 0.0
                            )
                            _ads = (
                                sum(prof_hist_sub_dist) / len(prof_hist_sub_dist)
                                if prof_hist_sub_dist
                                else 0.0
                            )
                            _aho = (
                                sum(prof_hist_sub_hof) / len(prof_hist_sub_hof)
                                if prof_hist_sub_hof
                                else 0.0
                            )
                            _alb = (
                                sum(prof_hist_sub_lbl) / len(prof_hist_sub_lbl)
                                if prof_hist_sub_lbl
                                else 0.0
                            )
                            _ayc = (
                                sum(prof_hist_sub_ycf) / len(prof_hist_sub_ycf)
                                if prof_hist_sub_ycf
                                else 0.0
                            )
                            tg_text = (
                                f"TG: TRK:{_atr:.1f} GM:{_agm:.1f} DS:{_ads:.1f} "
                                f"HO:{_aho:.1f} LB:{_alb:.1f} YC:{_ayc:.1f}ms"
                            )
                            cv2.putText(
                                frame,
                                tg_text,
                    ...
                    if cue_sender is not None and y_pos_arm is not None:
                        _k = ARM_CUE_SEND_TOGGLE_KEY
                        if cue_sender.enabled:
                            _arm_label = (
                                f"ARM CUE ({_k}) ON  {cue_sender.status_label()}"
                            )
                            _arm_color = (
                                (0, 220, 0)
                                if cue_sender.last_send_ok
                                else (0, 100, 255)
                            )
                        else:
                            _arm_label = f"ARM CUE ({_k}) OFF"
                            _arm_color = (140, 140, 150)
                        cv2.putText(
                            frame,
                            _arm_label,
                            (x_start, y_pos_arm),
                    ...
                    x_cam = x_guides + w_guides + spacing
                    x_fps = x_cam + w_cam + spacing
                    ...
                    # Shift right-side HUD slots left so they stay clear of HUD(H) at bottom-right.
                    x_queue = max(x_mode + w_mode + spacing, x_queue - hud_h_reserved_w)
                    x_acc = max(x_queue + w_queue + spacing, x_acc - hud_h_reserved_w)
                    x_yolo_px = max(x_acc + w_acc + spacing, x_yolo_px - hud_h_reserved_w)
                    ...
                    _cam_lbl, _cam_col = cam_hud_label_and_color(cam, not network_loss_mode, frame if not network_loss_mode else None)
                    cv2.putText(frame, _cam_lbl, (x_cam, y_pos), font, font_scale, _cam_col, thickness)
                    ...
                    _hud_h_text = "HUD(H): ON"
                    (_hud_hw, _), _ = cv2.getTextSize(_hud_h_text, font, font_scale, thickness)
                    _hud_h_x = max(x_start, w - margin_right - _hud_hw)
                    cv2.putText(
                        frame,
                        _hud_h_text,
                        (_hud_h_x, y_pos),
                ...
                prof_hist_sub_trk.append(prof_sub_trk_ms)
                prof_hist_sub_gm.append(prof_sub_gm_ms)
                prof_hist_sub_dist.append(prof_sub_dist_ms)
                prof_hist_sub_hof.append(prof_sub_hof_ms)
                prof_hist_sub_lbl.append(prof_sub_lbl_ms)
                prof_hist_sub_ycf.append(prof_sub_ycf_ms)
            ...
            if key == ord('q'): break
                ...
                if cue_sender is not None:
                    cue_sender.set_enabled(not cue_sender.enabled)
                    print(
                        f"🦾 ARM CUE LAN send ({ARM_CUE_SEND_TOGGLE_KEY}): "
                        f"{'ON' if cue_sender.enabled else 'OFF'}"
                    )
            elif key == ord('c'):
                ignore_motion_mask = None
                ignore_motion_mask_downsampled = None
