...
# --- Sound (เหมือน gun_aim_assist_vector: บีปสั้น + จังหวะเร่งตามวง) ---
SOUND_ON_READY = True
SOUND_FILE = "beep_2x.wav"
USE_SHORT_BEEP = True
SOUND_FILE_SHORT = "beep_short.wav"
BEEP_SHORT_DURATION_SEC = 0.06
SOUND_READY_INTERVAL = 0.05   # ตอน ready เล่นถี่มากจนฟังเหมือนเสียงยาว
SOUND_APPROACH_ON = True
# ช่วงบีป (วินาที) ต่อระดับวง: วงนอกสุด → วงถัดมา → วงใน (ใกล้ ready)
APPROACH_BEEP_INTERVAL_OUTER = 0.8
APPROACH_BEEP_INTERVAL_MID2 = 0.4
APPROACH_BEEP_INTERVAL_MID1 = 0.25
...
def create_short_beep_wav(filepath, duration_sec=0.06, sample_rate=44100, freq=880):
    """สร้างไฟล์ WAV เสียงบีปสั้น — ใช้ครั้งเดียวตอนสตาร์ท (เหมือน gun_aim_assist_vector)."""
...
def compute_approach_beep_interval(d, radius_px, min_side):
    """
    ช่วงเวลา (วินาที) ระหว่างบีปตามระดับวงรัศมี (เหมือน gun_aim_assist_vector):
    - วงนอกสุด = บีปห่าง, วงถัดมาเร่งขึ้น, วงในถี่มาก; วง ready = ไม่บีป (ใช้เสียง ready แทน)
    ...
    if d > r3:
        return None
    if d <= r1:
        return APPROACH_BEEP_INTERVAL_MID1
    if d <= r2:
        return APPROACH_BEEP_INTERVAL_MID2
    return APPROACH_BEEP_INTERVAL_OUTER
...
def main():
    ...
    while True:
        ...
        if app_mode == "normal":
            ...
            pad_bar = 8
            y_bottom = fh_d - 12
            y1_bar = max(0, y_bottom - th_msg_h - pad_bar)
            y2_bar = min(fh_d, y_bottom + pad_bar)
            cv2.rectangle(display, (0, y1_bar), (fw_d, y2_bar), (0, 0, 0), -1)
