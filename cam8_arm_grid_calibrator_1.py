...
Phase 1 — LIMIT SWEEP:
  Rotate arm to 4 extreme corners (top-left, top-right, bot-right, bot-left).
  At each corner: click the point on cam8 where cam4 is centred, then press C.
  → Defines the reachable polygon on cam8.

Phase 2 — CALIBRATION:
  Click a point inside reachable zone → move arm until cam4 centres on it → C.
  Repeat ≥9 points spread across the reachable zone → S to save.
...
Keys (all phases):
  C / Joystick B0   - confirm current point
...
GRID_COLS = 10
GRID_ROWS = 5
HYBRID_COARSE_GRID_COLS = 9
HYBRID_COARSE_GRID_ROWS = 5
HYBRID_FINE_GRID_COLS = 5
HYBRID_FINE_GRID_ROWS = 5
...
LIMIT_STEPS = [
    "1/4: Move arm to TOP-LEFT   -> click on cam8 where cam4 is centred -> C",
    "2/4: Move arm to TOP-RIGHT  -> click on cam8 where cam4 is centred -> C",
    "3/4: Move arm to BOT-RIGHT  -> click on cam8 where cam4 is centred -> C",
    "4/4: Move arm to BOT-LEFT   -> click on cam8 where cam4 is centred -> C",
]

# Large HUD: (headline, instruction) per limit step — avoids one huge overlapping line
LIMIT_STEPS_HUD: List[Tuple[str, str]] = [
    ("STEP 1/4 — ARM AT TOP-LEFT", "Click CAM8 where CAM4 center aims, then C"),
    ("STEP 2/4 — ARM AT TOP-RIGHT", "Click CAM8 where CAM4 center aims, then C"),
    ("STEP 3/4 — ARM AT BOTTOM-RIGHT", "Click CAM8 where CAM4 center aims, then C"),
    ("STEP 4/4 — ARM AT BOTTOM-LEFT", "Click CAM8 where CAM4 center aims, then C"),
]
...
def _on_mouse(event, x, y, flags, param):
    """Left-click on cam8 panel (top strip) — only active in 'limits' and 'calibration' phases."""
    ...
    if _state.cam8_w <= 0 or _state.cam8_h <= 0:
        return
    # Phase home: home position is fixed — no click needed
...
def _draw_reachable_polygon(disp8: np.ndarray) -> None:
    ...
    for i, pt in enumerate(_state.limit_corners):
        ...
        cv2.putText(disp8, f"L{i+1}", (dx + 8, dy - 4),
...
def _build_reachable_polygon() -> None:
    ...
    _state.reachable_poly_cam8 = cv2.convexHull(
        np.array(cpts, dtype=np.float32)).reshape(-1, 1, 2)
...
def _confirm_limit(arm) -> None:
    """Phase limits: record a limit corner."""
    if _state.cam8_click is None:
        print("[CAL] Click on cam8 first, then press C")
        return
    pan = getattr(arm, "pos_x", 0.0)
    tilt = getattr(arm, "pos_y", 0.0)
    label = ["top-left", "top-right", "bot-right", "bot-left"][
        min(_state.limit_step, 3)]
    c = {"cam8_px": float(_state.cam8_click[0]),
         "cam8_py": float(_state.cam8_click[1]),
         "arm_pan": float(pan), "arm_tilt": float(tilt), "label": label}
    _state.limit_corners.append(c)
    _state.cam8_click = None
    _state.limit_step += 1
    _build_reachable_polygon()
    print(f"[CAL] Limit {label}: cam8=({c['cam8_px']:.0f},{c['cam8_py']:.0f})  "
          f"arm=({c['arm_pan']:.2f},{c['arm_tilt']:.2f})")
    if _state.limit_step >= 4:
        print("[CAL] Limit sweep done! Press SPACE -> Phase 2: Calibration")


def _confirm_calibration(arm) -> None:
    """Phase calibration: record a cam8->arm pair."""
    if _state.cam8_click is None:
        print("[CAL] Click on cam8 first, then press C")
        return
...
def _hud_scales(panel_h: int) -> Tuple[float, float, int, int]:
    """Title scale, body scale, title thickness, body thickness from panel height."""
    k = max(0.55, min(1.2, panel_h / 380.0))
    title_s = 0.92 * k
    body_s = 0.62 * k
    tt = max(2, min(3, int(round(2 * k + 1))))
    bt = max(1, min(2, int(round(1.2 * k))))
    return title_s, body_s, tt, bt


def _hud_line_gap(panel_h: int) -> int:
    return max(8, int(panel_h * 0.025))
...
def _draw_cam8_labels(disp8: np.ndarray, pan: float, tilt: float) -> None:
    """Large phase HUD inside letterboxed cam8; stacked lines + dark panel (no overlap)."""
    off_x = _state.cam8_disp_off_x
    off_y = _state.cam8_disp_off_y
    nw = _state.cam8_disp_scaled_w
    nh = _state.cam8_disp_scaled_h
    if nw <= 0 or nh <= 0:
        return
    title_s, body_s, tt, bt = _hud_scales(nh)
    gap = _hud_line_gap(nh)
    meta_s = body_s * 0.88
    mt = max(1, bt - 1)
    phase = _state.phase
    specs: List[Tuple[str, float, int, Tuple[int, int, int]]] = []

    if phase == "home":
        specs.append(("PHASE 0 — REFERENCE", title_s, tt, (0, 200, 255)))
        if _state.home_confirmed:
            specs.append(("Reference saved — press SPACE for Phase 1", body_s, bt, (80, 255, 120)))
        else:
            specs.append(("Place target on yellow REF mark", body_s, bt, (255, 255, 0)))
            specs.append(("Center CAM4 crosshair on target, press C", body_s, bt, (255, 255, 0)))
        specs.append((f"Arm  pan {pan:+.1f}   tilt {tilt:+.1f}", meta_s, mt, (190, 190, 190)))
    elif phase == "limits":
        step = _state.limit_step
        specs.append(("PHASE 1 — LIMIT CORNERS", title_s, tt, (0, 220, 255)))
        if step < 4:
            h1, h2 = LIMIT_STEPS_HUD[step]
            specs.append((h1, body_s, bt, (0, 255, 255)))
            specs.append((h2, body_s, bt, (255, 255, 200)))
        else:
            specs.append(("All corners done", body_s, bt, (80, 255, 120)))
            specs.append(("Press SPACE for Phase 2", body_s, bt, (255, 255, 200)))
        specs.append(
            (f"Arm  pan {pan:+.1f}   tilt {tilt:+.1f}   corners {len(_state.limit_corners)}/4",
             meta_s, mt, (255, 230, 120))
        )
    else:
        specs.append(("PHASE 2 — CALIBRATION POINTS", title_s, tt, (80, 255, 140)))
        specs.append(("Click CAM8 inside green zone, aim CAM4, press C", body_s, bt, (255, 255, 220)))
        if _state.reachable_poly_cam8 is not None:
            specs.append(("Green = reachable   Orange dot = outside zone", meta_s, mt, (160, 200, 255)))
        else:
            specs.append(("No limit polygon — full frame treated reachable", meta_s, mt, (180, 180, 180)))
        specs.append(
            (f"Points {len(_state.points)}   Arm  pan {pan:+.1f}   tilt {tilt:+.1f}", meta_s, mt, (200, 200, 200))
        )

    x0 = off_x + 12
    pad = 10
    measured: List[Tuple[str, float, int, Tuple[int, int, int], int, int, int]] = []
    max_tw = 0
    total_h = 0
    for text, sc, thk, col in specs:
        (tw, th), bl = cv2.getTextSize(text, HUD_FONT, sc, thk)
        measured.append((text, sc, thk, col, tw, th, bl))
        max_tw = max(max_tw, tw)
        total_h += th + bl + gap
    total_h -= gap if measured else 0

    top_pad = off_y + 10
    bg_w = min(nw - 8, max_tw + 2 * pad)
    bg_h = min(nh - 8, total_h + 2 * pad)
    bg_x = off_x + 6
    bg_y = max(off_y + 4, top_pad - pad)
    _draw_hud_dark_panel(disp8, bg_x, bg_y, bg_w, bg_h, 0.5)

    y = top_pad
    for text, sc, thk, col, tw, th, bl in measured:
        y += th + bl
        cv2.putText(disp8, text, (x0, y), HUD_FONT, sc, col, thk, cv2.LINE_AA)
        y += gap


def _draw_recal_cam8_hud(
    disp8: np.ndarray,
    pan: float,
    tilt: float,
    recal_done: bool,
    ...
    """RECAL mode: large stacked HUD on cam8 panel."""
    off_x = _state.cam8_disp_off_x
    off_y = _state.cam8_disp_off_y
    nw = _state.cam8_disp_scaled_w
    nh = _state.cam8_disp_scaled_h
    ...
    mt = max(1, bt - 1)
    specs: List[Tuple[str, float, int, Tuple[int, int, int]]] = [
        ("RECAL — REFERENCE UPDATE", title_s, tt, (0, 200, 255)),
    ]
    if recal_done:
        specs.append((f"Delta  pan {delta_pan:+.2f}   tilt {delta_tilt:+.2f}", body_s, bt, (100, 255, 180)))
        specs.append((f"{n_pts} points updated   Refine {n_refine}   Press S to save", body_s, bt, (255, 255, 200)))
    else:
        specs.append(("Click CAM8 to refine, or press C with no click for REF shift", body_s, bt, (255, 255, 200)))
        specs.append(
            (f"Stored ref arm  pan {stored_home_pan:+.1f}   tilt {stored_home_tilt:+.1f}", meta_s, mt, (200, 200, 160))
        )
    specs.append((f"Live arm  pan {pan:+.1f}   tilt {tilt:+.1f}", meta_s, mt, (190, 190, 190)))
    specs.append(
        (
            f"TEST MODE: {'ON' if test_mode else 'OFF'}  (press T)",
            meta_s,
            mt,
            ((0, 255, 255) if test_mode else (140, 140, 140)),
        )
    )
...
def _draw_recal_status_bar(
    display: np.ndarray, disp_w: int, disp_h: int, saved: bool
) -> None:
    scale = max(0.58, min(0.88, disp_h / 620.0))
    thick = max(2, int(round(scale * 1.8)))
    gap = max(7, int(disp_h * 0.014))
    lines = [
        "T = test mode     Click CAM8     C = confirm     S = save     Q = quit",
        "H = home ref     D = delete refine     R = reset refine list",
    ]
    if saved:
        lines.append("[ SAVED ]")
    ...
    heights = 0
    for ln in lines:
        (_, th), bl = cv2.getTextSize(ln, HUD_FONT, scale, thick)
        heights += th + bl + gap
    heights -= gap
    strip_h = heights + 18
    strip_y = max(0, disp_h - strip_h)
    _draw_hud_dark_panel(display, 0, strip_y, disp_w, disp_h - strip_y, 0.55)

    y = disp_h - 12
    for ln in reversed(lines):
        (_, th), bl = cv2.getTextSize(ln, HUD_FONT, scale, thick)
        cv2.putText(
            display, ln, (14, y), HUD_FONT, scale, (230, 230, 230), thick, cv2.LINE_AA
        )
        y -= th + bl + gap
...
def _draw_cam4_panel_hud(
    ...
    """Large top HUD on CAM4 panel (does not overlap crosshair)."""
    title_s, body_s, tt, bt = _hud_scales(panel_h)
    gap = _hud_line_gap(panel_h)
    lines: List[Tuple[str, float, int, Tuple[int, int, int]]] = [
        ("CAM4 — ARM CAMERA", title_s, tt, (0, 255, 255)),
        ("Aim target at yellow crosshair center", body_s, bt, (255, 255, 220)),
        (f"Arm  pan {pan:+.1f}   tilt {tilt:+.1f}", body_s * 0.9, max(1, bt - 1), (200, 200, 200)),
    ]
    if extra_line:
        lines.append((extra_line, body_s * 0.88, bt, (0, 200, 255)))
    ...
    bg_w = min(disp4.shape[1] - 12, max_tw + 2 * pad)
    bg_h = min(panel_h // 2, total_h + 2 * pad)
    _draw_hud_dark_panel(disp4, 8, top - 4, bg_w, bg_h, 0.5)

    y = top
    for text, sc, thk, col, tw, th, bl in measured:
        y += th + bl
        cv2.putText(disp4, text, (x0, y), HUD_FONT, sc, col, thk, cv2.LINE_AA)
        y += gap


def _draw_status_bar(display: np.ndarray, disp_w: int, disp_h: int,
                     saved: bool, now: float,
                     warn_msg: Optional[str] = None, warn_until: float = 0.0) -> None:
    phase = _state.phase
    scale = max(0.58, min(0.92, disp_h / 620.0))
    thick = max(2, int(round(scale * 1.8)))
    gap = max(7, int(disp_h * 0.014))

    if phase == "home":
        line1 = "C = confirm reference     SPACE = next phase     Q = quit"
        line2 = "H = move arm to saved reference"
    elif phase == "limits":
        line1 = "C = confirm this corner     SPACE = go Phase 2     Q = quit"
        line2 = "D = delete last corner     L = redo limit sweep"
    else:
        line1 = "C = save point     S = save JSON file     Q = quit"
        line2 = "D = delete point     R = reset points     H / L = ref / limits"

    lines = [line1, line2]
    if saved:
        lines.append("[ CALIBRATION SAVED ]")
    ...
    if warn_msg and now < warn_until:
        ws = scale * 1.05
        wt = min(3, thick + 1)
        (_, wh), wbl = cv2.getTextSize(warn_msg, HUD_FONT, ws, wt)
        warn_h = wh + wbl + gap + 4

    strip_h = heights + 18 + warn_h
    strip_y = max(0, disp_h - strip_h)
    _draw_hud_dark_panel(display, 0, strip_y, disp_w, disp_h - strip_y, 0.55)
    ...
    if warn_msg and now < warn_until:
        ...
        wy = strip_y + wh + wbl + 6
        cv2.putText(
            display, warn_msg, (14, wy), HUD_FONT, ws, (60, 80, 255), wt, cv2.LINE_AA
        )
...
def _run_loop(arm, cam8, cam4, joystick, joy_mapper, mode: str = "full") -> None:
    ...
    while True:
        ...
        _draw_cam8_labels(disp8, pan, tilt)
        ...
        _recal_extra = (
            "RECAL: aim REF at crosshair, then C" if mode == "recal" else None
        )
        _draw_cam4_panel_hud(disp4, bottom_h, pan, tilt, extra_line=_recal_extra)
        _draw_cam4_crosshair(disp4, canvas_w, bottom_h, cam4_map)

        display = np.vstack([disp8, disp4])
        _draw_status_bar(display, canvas_w, canvas_h, saved, now,
                         warn_msg, warn_until)
        ...
        if key in (ord("q"), 27):
            ...
            if _state.phase == "home":
                ...
                _state.limit_step = max(0, _state.limit_step - 1)
                _build_reachable_polygon()
                print(f"[CAL] Deleted limit: {removed['label']}")
                saved = False
            elif _state.phase == "calibration" and _state.points:
                _state.points.pop()
                print(f"[CAL] Deleted calibration point (remaining: {len(_state.points)})")
                saved = False
            ...
            _state.phase = "limits"
            saved = False
            print("[CAL] Limit sweep reset → Phase 1")
            ...
            if _state.phase == "home":
                ...
                _state.phase = "calibration"
                if _state.limit_step < 4:
                    print(f"[CAL] Limit sweep skipped ({_state.limit_step}/4) -> Phase 2")
                else:
                    print("[CAL] -> Phase 2: CALIBRATION")
            # in calibration: SPACE does nothing
...
def run_recal(arm, cam8, cam4, joystick, joy_mapper,
    ...
    while True:
        ...
        _draw_cam4_panel_hud(
            disp4,
            bottom_h,
            pan,
            tilt,
            extra_line="RECAL: aim REF at crosshair, then C",
        )
        _draw_cam4_crosshair(disp4, canvas_w, bottom_h, cam4_map)

        display = np.vstack([disp8, disp4])
        _draw_recal_status_bar(display, canvas_w, canvas_h, saved)
        cv2.imshow(win_name, display)
...
def _print_full_help() -> None:
    ...
    print("Phase 1 LIMIT SWEEP:")
    for s in LIMIT_STEPS:
        print(f"  {s}")
    print("  SPACE/Enter -> Phase 2")
    print("Phase 2 CALIBRATION:")
    print("  Click cam8 (top panel) -> move arm -> C  (repeat >=9 points) -> S to save\n")
