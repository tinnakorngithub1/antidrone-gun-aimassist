...
class StitchedCameraStream:
    ...
    def __init__(self, base_module, left_camera, right_camera, preset_path, clone_right_from_left=False):
    ...
    def compose_frames(self, left_frame, right_frame):
        self._ensure_defaults(left_frame, right_frame)
        ...
        canvas_shape = (output_height, output_width, 3)
        left_rect_data = self._compute_paste_rect(
            left_img,
            int(left_offset.get("x", 0)),
            int(left_offset.get("y", 0)),
            canvas_shape,
        )
        right_rect_data = self._compute_paste_rect(
            right_img,
            int(right_offset.get("x", 0)),
            int(right_offset.get("y", 0)),
            canvas_shape,
        )
        ...
        if (
            ...
            and left_rect_data["h"] == output_height
            and right_rect_data["h"] == output_height
            and left_rect_data["x"] == 0
            and right_rect_data["x"] == left_rect_data["w"]
            and output_width == left_rect_data["w"] + right_rect_data["w"]
        ):
            stitched = np.hstack(
                [
        ...
        self.last_layout = {"left": left_rect, "right": right_rect}
        self.last_canvas_shape = (output_height, output_width)
        return canvas

    def read(self):
        left_active, left_frame, left_timestamp = self._read_stream(self.left_stream)
        ...
        if self.clone_right_from_left:
            ...
            right_active, right_frame, right_timestamp = self._read_stream(self.right_stream)
    ...
    def _draw_fps_banner(self, image, text):
        banner_h = max(44, int(round(image.shape[0] * 0.08)))
        cv2.rectangle(image, (0, 0), (image.shape[1] - 1, min(image.shape[0] - 1, banner_h)), (235, 235, 235), -1)
        font_scale = max(0.75, min(1.25, image.shape[1] / 1400.0))
        thickness = 2
        x = 16
        y = min(image.shape[0] - 12, int(round(banner_h * 0.68)))
        cv2.putText(image, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, lineType=cv2.LINE_AA)

    def overlay_display(self, image):
        self.last_display_shape = image.shape[:2]
        overlay = image.copy()
        fps_line = self._fps_text(image.shape)
        self._draw_fps_banner(overlay, fps_line)
        self._maybe_log_fps()
        if not self.edit_mode:
            return overlay
        ...
        lines = [
            ...
            "Keys: 1/2/3=target  C/V/B=action  WASD=adjust  Shift+WASD=step10  Enter=save+lock  G=reload R=reset-full M=toggle",
...
def build_bottom_synthetic_config(module, worker_config):
    ...
    width = int(preset.get("output_width", 0)) or int(left_cfg.get("width", 1280)) * 2
    height = int(preset.get("output_height", 0)) or int(left_cfg.get("height", 720))
    return {
        **left_cfg,
...
def run_bottom_stitch_worker(
    ...
    del apply_worker_thresholds, patch_runtime_for_worker, hook_proxy_cls, single_camera_main

    frame_queue = worker_config.get("frame_queue")
    control_queue = worker_config.get("control_queue")
    layout = worker_config.get("layout", {})
    window_name = worker_config.get("window_name", "bottom-stitched")
    ...
    def handle_control_events():
        ...
        while True:
            ...
            if event_type == "key":
                ...
                if stitched_stream.handle_key(key):
                    continue
                if key in (ord("q"), 27):
                    should_exit = True
                    break
    ...
    try:
        while True:
            ...
            module.current_frame_w = frame_w
            module.current_frame_h = frame_h

            if queue_display_mode:
                display_w = max(1, int(layout.get("width", frame_w)))
                display_h = max(1, int(layout.get("height", frame_h)))
            else:
                display_w, display_h = _fit_display_size(frame_w, frame_h)

            module.display_w = display_w
            module.display_h = display_h

            display_frame = frame
            if display_w != frame_w or display_h != frame_h:
...
def run_standalone_editor(
    ...
    try:
        while True:
            ...
            display_w, display_h = _fit_display_size(frame.shape[1], frame.shape[0])
            module.display_w = display_w
            module.display_h = display_h
