...
PPD_HALF_SPREAD_MAX_PCT = 3.0   # ppd ครึ่งแรก vs ครึ่งหลังต่างกันเกินนี้ = parallax (ฉากใกล้)
...
LAT_CANDIDATES = np.arange(0.0, 0.501, 0.0025)  # ลอง L 0–500ms (4K H264 บน Jetson หน่วงได้ถึง 300ms+)

NOISE_SEC = 8.0           # เก็บ bbox center กี่วินาที

STEPS = ["prep", "ppd", "latency", "noise", "boresight", "done"]
...
def enter(window_name: str, camera_name: str, frame_w: int, frame_h: int,
    ...
    _W.log = [f"camera: {camera_name}  ({frame_w}x{frame_h})"]
    _W.err = ""
    _W.busy = False
...
def hud_label() -> str:
    return f"WIZARD [{STEPS[_W.step]}]" + ("  measuring..." if _W.busy else "")
...
def _sweep_axis(arm, axis: int, scale: float) -> Tuple[List[float], List[float]]:
    ...
    if len(steps) < 4:
        raise RuntimeError("axis limits too narrow to sweep")
    ...
    for tgt in steps[1:]:            # ก้าวละ PPD_STEP_DEG เท่ากันหมด → shift เล็ก วัดแม่น
        ...
        if img is None:
            raise RuntimeError("camera stopped sending frames mid-measurement")
        dx, dy, resp = _shift(prev_img, img)
        if resp < 0.05:
            raise RuntimeError(f"image match failed (response {resp:.2f}) - scene has no texture?")
        acc_px += (dx if axis == 0 else dy) / scale     # กลับเป็นพิกเซลของภาพเต็ม
        prev_img = img
        d_deg.append(actual - start_deg)
        d_px.append(acc_px)
        _say(f"  {'pan' if axis==0 else 'tilt'} {actual - start_deg:+.2f}deg -> {acc_px:+.1f}px")
...
def _check_texture(gray: np.ndarray) -> None:
    ...
    if lap < 40.0:
        raise RuntimeError(
            f"scene too flat (edge energy {lap:.0f}) - empty sky / dark / lens covered? "
            "Point at trees or buildings, or press C for click mode"
        )
...
def _worker_ppd(arm) -> None:
    try:
        g = _grab()
        if g is None:
            raise RuntimeError("no frame from camera yet")
        ...
        if len(dpan) < 4 or len(dtilt) < 4:
            raise RuntimeError("not enough sample points (arm limits too narrow?)")

        # --- parallax guard ---
        # ppd เป็นสมบัติของ 'เลนส์' → ต้องเท่ากันไม่ว่าเล็งไปทางไหน
        # แต่ถ้ากล้องไม่ได้อยู่บนแกนหมุน (offset d) พอแขนหมุน กล้องจะ 'เลื่อน' ด้วย ไม่ใช่แค่หมุน
        # → ฉากที่ระยะ R ขยับเพิ่มอีก d/R → ppd ที่วัดได้ = ppd_จริง × (1 + d/R)
        # → ยิ่งฉากใกล้ยิ่งเพี้ยน และ 'เพี้ยนไม่เท่ากัน' ถ้าฉากแต่ละส่วนอยู่คนละระยะ
        # ตรวจได้: fit ครึ่งแรกกับครึ่งหลังของ sweep แยกกัน — ถ้าต่างกันมาก = ฉากใกล้/ลึกไม่เท่ากัน
        # (r² สูงจับไม่ได้ เพราะแต่ละครึ่งยังเป็นเส้นตรงสวย ๆ อยู่ แค่คนละความชัน)
        _bad = []
        for _name, _dd, _dp in (("pan", dpan, dpx), ("tilt", dtilt, dpy)):
            _h = len(_dd) // 2
            _m1, _ = _fit_through_origin(_dd[:_h], _dp[:_h])
            _m2, _ = _fit_through_origin(_dd[_h:], _dp[_h:])
            if abs(_m1) > 1.0 and abs(_m2) > 1.0:
                _spread = abs(_m1 - _m2) / max(abs(_m1), abs(_m2)) * 100.0
                _say(f"   {_name}: ครึ่งแรก {abs(_m1):.1f} / ครึ่งหลัง {abs(_m2):.1f} px/deg (ต่าง {_spread:.1f}%)")
        ...
        if r2x < PPD_MIN_R2 or r2y < PPD_MIN_R2:
            raise RuntimeError(
                f"relationship not linear enough (r2={r2x:.3f}/{r2y:.3f}) - "
                "scene may be empty sky / dark / has moving objects. Press C for click mode"
            )
        if abs(ppd_x) < 1.0 or abs(ppd_y) < 1.0:
            raise RuntimeError(f"ppd looks wrong ({ppd_x:.1f}/{ppd_y:.1f}) - did the arm actually move?")

        _W.ppd_x, _W.ppd_y = ppd_x, ppd_y
        _W.ppd_r2 = (r2x, r2y)
        fh = _W.frame_w / abs(ppd_x)
        fv = _W.frame_h / abs(ppd_y)
        _say(f"OK ppd = {ppd_x:.2f} / {ppd_y:.2f} px/deg  (r2 {r2x:.4f}/{r2y:.4f})")
        _say(f"   -> real FOV {fh:.1f} x {fv:.1f} deg")
        if ppd_y > 0:
            _say("WARN ppd_y is positive - breaks convention (system assumes negative). Check camera mounting")
...
def _worker_latency(arm) -> None:
    try:
        if not _W.ppd_x:
            raise RuntimeError("measure ppd first")
        scale = PROC_W / float(_W.frame_w)
        lim = arm._effective_x_limits
        amp = min(LAT_AMP_DEG, (lim[1] - lim[0]) / 2.0 - 1.0)
        if amp < 1.0:
            raise RuntimeError("pan limits too narrow to sweep")
        ...
        if len(samples) < 40:
            raise RuntimeError(f"only {len(samples)} frames captured - frame rate too low")
        if state["bad"] > len(samples) * 0.3:
            raise RuntimeError("image match failing often - scene has no texture?")
        ...
        if float(np.ptp(pans)) < 1.0:
            raise RuntimeError("arm barely moved - is it connected and unlocked?")
        ...
        if i == 0 or i == len(LAT_CANDIDATES) - 1:
            _say(f"WARN result hit the edge of the search range ({best_L*1000:.0f}ms) - unreliable")
        else:
...
def _worker_noise(arm) -> None:
    try:
        if _W.get_detection_center is None:
            raise RuntimeError("no detection feed")
        _say(f"aim at the target (arm still) - sampling {NOISE_SEC:.0f}s ...")
        t0 = time.time()
        xs: List[float] = []
        ys: List[float] = []
        tt: List[float] = []
        while time.time() - t0 < NOISE_SEC:
            ...
            time.sleep(0.02)
        if len(xs) < 20:
            raise RuntimeError(f"only {len(xs)} frames on target - need a target in frame with LOCK held")
        ...
        _say(f"OK sigma = {sigma:.1f} px  (= {sigma/ppd:.3f}deg at ppd {ppd:.0f}) from {len(xs)} frames")
        _say(f"   -> LOCK_MEAS_SIGMA_PX = {sigma:.1f}  (Kalman R = (sigma/ppd)^2)")
    except Exception as e:
        _W.err = str(e)
        _say(f"✗ {e}")
    finally:
...
def _consume_click(arm) -> None:
    ...
    if _W.step == 4:   # boresight
        if not _W.ppd_x or not _W.ppd_y:
            _say("ERR need ppd first")
            return
        cx, cy = _W.frame_w / 2.0, _W.frame_h / 2.0
        _W.boresight = ((px - cx) / _W.ppd_x, (py - cy) / _W.ppd_y)
        _say(f"OK boresight = {_W.boresight[0]:+.2f} / {_W.boresight[1]:+.2f} deg "
             f"(from click at {px:.0f},{py:.0f})")
        return
    ...
    def _go():
        try:
            ...
            _say(f"point {len(_W.click_pts)}: px({px:.0f},{py:.0f}) <- arm({arm.pos_x:+.2f},{arm.pos_y:+.2f})")
            _say("   fine-tune with arrows until it is dead center, then Enter to confirm (or click again)")
        finally:
            _W.busy = False
...
def _fit_click_pts() -> bool:
    if len(_W.click_pts) < 3:
        _say("ERR need at least 3 points")
        return False
    cx, cy = _W.frame_w / 2.0, _W.frame_h / 2.0
    ...
    if abs(mx) < 1.0 or abs(my) < 1.0:
        _say("ERR result looks wrong")
        return False
    _W.ppd_x, _W.ppd_y, _W.ppd_r2 = mx, my, (r2x, r2y)
    _say(f"OK ppd (click mode) = {mx:.2f} / {my:.2f}  (r2 {r2x:.3f}/{r2y:.3f}, {len(_W.click_pts)} points)")
    return True
...
def save_all() -> List[str]:
    ...
    if _W.latency is not None or _W.sigma_px is not None:
        try:
            ...
            _say(f"WARN cannot write runtime_config: {e}")
    return written
...
def _draw(frame: np.ndarray) -> None:
    ...
    if _W.ppd_x:
        ...
        row("-> real FOV", f"{fh:.1f} x {fv:.1f} deg", C_OK)
    else:
        row("ppd", "not measured", C_DIM)
    row("ego latency", f"{_W.latency*1000:.0f} ms" if _W.latency is not None else "not measured",
        C_OK if _W.latency is not None else C_DIM)
    row("bbox jitter sigma", f"{_W.sigma_px:.1f} px" if _W.sigma_px is not None else "not measured",
        C_OK if _W.sigma_px is not None else C_DIM)
    row("boresight", f"{_W.boresight[0]:+.2f} / {_W.boresight[1]:+.2f} deg" if _W.boresight else "not set",
    ...
    for line in _W.log[-9:]:
        col = C_ERR if line.startswith("ERR") else (C_OK if line.startswith("OK") else
              (C_WARN if line.startswith("WARN") else C_DIM))
    ...
    ht.put_text(frame, "Space=run  N=next  B=back  C=toggle click mode  S=save  Esc/W=exit", (x0 + int(18 * s), y1 - int(18 * s)), fs * 0.78, C_DIM, th)
...
def _step_hint() -> str:
    if _W.busy:
        return "measuring... do not move the camera or arm"
    if _W.step == 0:
        return "Prep: arm connected + unlocked, scene has texture (trees/buildings) not empty sky -> Space"
    if _W.step == 1:
        if _W.click_mode:
            return f"Click mode: click distinct points ({len(_W.click_pts)} so far) -> Enter=fit  |  C=back to auto"
        return "Space = auto-measure ppd (arm sweeps +/-9 deg)  |  C = click mode if scene has no texture"
    if _W.step == 2:
        return f"Space = measure ego latency (arm swings a sine +/-{LAT_AMP_DEG:.0f} deg for {LAT_DURATION_SEC:.0f}s)"
    if _W.step == 3:
        return "Get a target in frame with LOCK held, then Space = measure noise (arm must be still)"
    if _W.step == 4:
        return "Boresight: aim the barrel at a reference point, then click where it actually points"
    return "S = save all (ppd/boresight -> calibration_data, latency/sigma -> runtime_config)"
...
def handle_key(key: int, arm) -> str:
    ...
    if key in (ord("n"), ord("N")):
        ...
        _W.click_pts = []
        _say("click mode" if _W.click_mode else "auto mode")
    elif key in (13, 10) and _W.step == 1 and _W.click_mode:
        _fit_click_pts()
    elif key in (ord("s"), ord("S")):
        files = save_all()
        _say(("OK saved: " + ", ".join(files)) if files else "ERR nothing to save")
        if files:
            _say("   ppd changed -> FOV/Kalman R/boresight are re-derived automatically on exit")
            return "saved"
    elif key == 32:   # Space = รันขั้นตอนปัจจุบัน
        if arm is None:
            _say("ERR no arm - cannot calibrate")
            return "none"
        if getattr(arm, "is_simulation_mode", False):
            _say("ERR arm is in simulation mode - need the real arm")
            return "none"
        if _W.step == 0:
            _say("arm ready - go to ppd step (N)")
            _W.step = 1
