...
def _move_arm_to_target_center(
    ...
    if use_homography_lock:
        ...
        if res_center_lock is not None and res_lock is not None:
            # มุมที่ต้องหมุน (จากศูนย์เล็งไปโดรน ใน arm space) — ใช้ move_relative เพื่อไม่พึ่ง pos จาก GRBL
            delta_pan_lock = res_lock[0] - res_center_lock[0]
            delta_tilt_lock = res_lock[1] - res_center_lock[1]
            try:
                arm_controller.move_relative(delta_pan_lock, delta_tilt_lock, blocking=True)
                did_move = True
            except Exception:
                pass
    if not did_move:
