...
def _apply_arm_move_relative(arm: Any, delta_pan: float, delta_tilt: float) -> None:
    ...
    if arm is None or getattr(arm, "is_simulation_mode", False):
        return
    if SWAP_PAN_TILT:
...
def _variable_step_toward_target(
    ...
    step_mm = min(error_mm, max_step_by_feed)
    # แขนต้องเร็วกว่าเป้า: อย่างน้อย = ความเร็วเป้า × dt × ตัวคูณ
    min_step_arm = min_step_to_match_target_mm * CONTINUOUS_ARM_FASTER_THAN_TARGET_FACTOR
    step_mm = max(step_mm, min_step_arm, STEP_MIN_MM)
...
def _on_mouse(event: int, x: int, y: int, flags: int, param: Any) -> None:
    state = _state
    # โหมด L Manual: LBUTTONUP — เลิกลาก
...
def main() -> None:
    ...
    try:
        while True:
            ...
            # โหมด L Auto: เป้าจากจำลองการบินโดรน — ความเร็วไม่คงที่, ส่งเป้าให้แขนเฉพาะเมื่อ detection เจอ (ไม่รันเมื่อ sim_drag_active หรือ Manual)
            if (
                state.mode == "test"
                and getattr(state, "test_track_sim", False)
                ...
                import random
                # โหมด speed lock: เร็วเดี่ยว / ช้าเดี่ยว / หยุดเดี่ยว; ไม่ล็อก = สลับความเร็วสุ่มทุก 2–6 วินาที
                ...
                # ช่วงความเร็วกว้างขึ้น: stop=0, slow=0.5, medium=1.5, fast=4.0
                ...
                x_lo, x_hi = x_lim[0], x_lim[1]
                y_lo, y_hi = y_lim[0], y_lim[1]
                # แสดงผลจุดโดรนทุกเฟรม
                if state.output_w > 0 and state.output_h > 0:
                ...
                # เป้าแขน: มี px_per_deg ใช้ Kalman ทำนายตำแหน่งล่วงหน้า (ดักหน้า) — ลดการสั่นกระตุก
                if (
                    state.output_w > 0
                    and state.output_h > 0
                    and state.sim_display_px is not None
                    and state.sim_display_py is not None
                    and state.px_per_deg_x is not None
                    and state.px_per_deg_y is not None
                ):
                    ...
                    # ไม่มี px_per_deg: ส่งเป้าจากตำแหน่ง sim ปัจจุบันทุก detection_interval
            ...
            if (
                state.mode == "test"
                and (getattr(state, "test_continuous", False) or getattr(state, "test_track_sim", False) or getattr(state, "test_teaching", False))
            ...
            if state.mode == "calibration":
                ...
                keys_txt = "Keys: [J]=teach [A]=Click/Auto [M]=Model [F]=Retrain [P]=cont [L]=sim [V]=plot [H]=home [R]=recal [S]=Save [C]=Calib [ESC]=Quit"
            ...
            if state.mode == "test" and getattr(state, "test_track_sim", False):
                ...
                cv2.putText(
                    frame, f"L: Sim — {auto_manual} [A]  speed: {speed_label} [K]  track: {track_style} [J]  [{pidx}] {pname}  0-9 ] [L]=off",
                    (10, y_l1), cv2.FONT_HERSHEY_SIMPLEX, FONT_HUD, COLOR_HUD_RED, THICKNESS_THIN
                )
                if not getattr(state, "track_sim_arm_auto", True):
                    cv2.putText(
                        frame, "Drag = drone flight, arm follows",
            ...
            if key in (KEY_ARROW_LEFT, KEY_ARROW_LEFT_MAC):
                arm.move_relative(-ARROW_STEP_DEG, 0.0, blocking=False)
            elif key in (KEY_ARROW_RIGHT, KEY_ARROW_RIGHT_MAC):
                arm.move_relative(ARROW_STEP_DEG, 0.0, blocking=False)
            elif key in (KEY_ARROW_UP, KEY_ARROW_UP_MAC):
                arm.move_relative(0.0, ARROW_STEP_DEG, blocking=False)
            elif key in (KEY_ARROW_DOWN, KEY_ARROW_DOWN_MAC):
                arm.move_relative(0.0, -ARROW_STEP_DEG, blocking=False)
    finally:
