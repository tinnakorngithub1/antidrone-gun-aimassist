...
CSRT_UPDATE_EVERY_N_FRAMES = 2  # รัน tracker ทุก N เฟรม; เฟรมอื่นถือตำแหน่งล่าสุด (ไม่ predict)
...
def main() -> None:
    ...
    try:
        while True:
            ...
            # Update CSRT every N frames; position (red dot / vector) only changes when CSRT returns bbox (no Kalman predict)
